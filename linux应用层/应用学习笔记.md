...
# 4、标准I/O库
## 4.1、标准输入、标准输出、标准错误
每个进程启动之后都会默认打开标准输入、标准输出以及标准错误，得到三个文件描述符，即 0、 1、2，其中 0 代表标准输入、 1 代表标准输出、 2 代表标准错误.在应用编程中可以使用宏：  
`STDIN_FILENO`、`STDOUT_FILENO`、`STDERR_FILENO`  

## 4.2、fopen() 打开文件
在系统调用是用open()来打开或创建文件，而在库函数中是用fopen()打开或创建函数  
```c
#include <stdio.h>
/*
path:文件路径
mode:读写权限
     r:只读   r+:可读，可写
     w:只写，不存在则创建   w+:可读，可写，不存在则创建
     a:只写，打开以后末尾追加    a+:可读可写，末尾追加

return: 成功：返回指向FILE类型对象的指针(FILE*)
        失败：返回NULL
*/
...
## 4.4、读文件fread()和写文件fwrite()和ferror()
```c
/*
ptr:  读到的数据 放入缓冲区
size:从文件读取 nmemb 个数据项，每一个数据项的大小为 size 个字节
nmemb:参数 nmemb 指定了读取数据项的个数
stream： FILE 指针
return: 成功: 返回读取到的数据项的数目
        失败：如果发生错误或到达文件末尾，则 fread()返回的值将小于参数 nmem
*/
...
nmemb： 要写入的元素个数
stream： 目标文件流
return: 成功: 返回写入的数据项的数目
        错误： fwrite()返回的值将小于参数 nmemb（或者等于 0）
*/
...
stream: FILE 指针
offset: 偏移量
whence: whence 用于定义参数 offset 偏移量对应的参考值
        SEEK_SET：从文件开头算，新位置=offset
        SEEK_CUR：从当前位置算，新位置=current + offset
        SEEK_END：从文件末尾算，新位置=file_size + offset 
return: 成功 0   失败 -1
